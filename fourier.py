...
class Fourier(object):
    ...
    def __init__(self, time, data, window=scipy.signal.boxcar):
        ...
        assert (len(self.time) == len(self.tdata),
    ...
    def stft(self, window_size, hop, window_fn=flattop, ra=[0, -1], filt=None):
        ...
        time = np.array([self.time[j + int(window_size / 2)]
                         for j in range(ra[0], ra[1] - window_size, hop)])
        # remove the DC average
        if filt is not None and filt in ['constant', 'linear']:
            chi = np.array([np.fft.rfft(w *
                                        (scipy.signal.detrend(self.tdata[i:i +
                                                                         window_size], type=filt))) *
                            2. /
                            window_size for i in range(ra[0], ra[1] -
                                                       window_size, hop)])
        elif filt is None:
            chi = np.array([np.fft.rfft(w *
                                        (self.tdata[i:i +
